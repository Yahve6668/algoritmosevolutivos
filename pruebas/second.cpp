...
std::vector < std::vector < int >> comsol(auto &a,int n){ 
   for (int i = 0; i < a.size(); ++i){ 
   	for (int j = 0; j < a[i].size() / 2; j += 2){ 
       std::swap(a[i][j] , a[i][ j + a[i].size() / 2] ); 
   	} 
   } 
   return a; 
} 
    ...
    for(int i=0;;++i){ 
    	 if(s[i].second <= s[ s.size() / 2 ].second){ 
          for(int j= 0 ;j < conjunto[s[i].first].size() ; ++ j ){ 
              conjunto[s[i].first][j] = ((conjunto[s[i].first][j] * 100) % n ); 
          } 
    	 }else{ 
    	 	break; 
    	 } 
...
std::vector < std::vector < int >> b(std::vector < std::vector < int >> & conjunto, int tam) { 
  ...
  for (int i = 0; i < tam / 2; ++i) { 
    r.push_back(conjunto[(evaludor[i].first + 1) % conjunto.size()]); 
  } 
