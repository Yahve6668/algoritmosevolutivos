...
BETA = 10.0   # Influencia de la distancia
...
def actualizar_feromonas(recorridos, longitudes, feromonas):
    ...
    for x in range(len(recorridos)):
        ...
        print("feromonas")
        print(longitudes[ciudad_actual])
        print(ciudad_actual)
        print(delta_feromona)
        print(feromonas[ciudad_actual])
...
def seleccionar_proxima_ciudad(it,ciudad_actual,tam, visitadas, distancias, feromonas,puntos_medios):
    ...
    print(ciudad_actual)
    #print(puntos_medios[ciudad_actual])
    ...
    for j in range(maxpor):
        if ( distancias[ciudad_actual] != float('inf')):
            ...
            print(distancias[s])
            print(feromonas)
            print((1 + distancias[ s ]) ** BETA)
            print((feromonas[s] ** ALPHA))
            print(((1.0 / (1 + distancias[ s ]) ** BETA)))

            print( (feromonas[s] ** ALPHA) * ((1.0 / (1 + distancias[ s ]) ** BETA)))
...
def aco(distancias, puntos_medios,puntos_medios_vistos,puntos):
    ...
    for iteracion in range(MAX_ITERACIONES):
        ...
        for k in range(NUM_HORMIGAS):
            ...
            print(proxima_ciudad)
            

            print("la ciudad c se jalo a la de ",end=" ")
            print(ciudad_actual,end=" ")
            print((ciudad_actual if ciudad_actual == proxima_ciudad else proxima_ciudad ))
            print("HOLAA") 
            print("recorrido")
...
"""            if(puntos_medios_vistos[puntos_medios[ciudad_inicial]] == float("inf")):
                puntos_medios_vistos[puntos_medios[ciudad_inicial]] = True
